...
[https://github.com/ElebetelNegusse2027/Negusse_Elebetel_201_A06](https://github.com/ElebetelNegusse2027/Negusse_Elebetel_201_A06)
...
player_points_per_game <- list()
for( i in names(player_info)){
  player <- player_info[[i]]
  calc_points <- player$points/player$games_played
  player_points_per_game[[i]]<- calc_points
}
...
```{r}
round_points <- sapply(player_points_per_game, function(i){round(i, 2)})
round_points
...
round_number<- apply(mat_rand, MARGIN = 1:2, round_)
round_number
```
...
```{r}
med_annual_temp<- apply(colorado_temp[, 3:14], 1, median)

max_temp<- which.max(med_annual_temp)
min_temp<- which.min(med_annual_temp)

median_max_year<- colorado_temp$year[max_temp]
median_min_year<- colorado_temp$year[min_temp]


print(paste0("The maximum median annual temperature of ", med_annual_temp[max_temp], " occured in ", median_max_year, " and the minimum median annual temperature of ", med_annual_temp[min_temp], " occured in ", median_min_year, "."))
```
...
sum_<- function(number){
  if(number ==0){
    ...
    last_dig<- number %% 10 
    remaining_num <- number %/% 10
    return(last_dig + sum_(remaining_num))
  }
}
result<- sum_(935068)
result
